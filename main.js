...
class AIStudyPlatform {
    ...
    generateAIResponse(type, message) {
        ...
        if (type === 'questoes') {
            ...
            } else if (lowerCaseMessage.includes("equação de segundo grau")) {
                response = "Uma equação de segundo grau é do tipo ax² + bx + c = 0, onde 'a' é diferente de zero. Para resolvê-la, usamos a Fórmula de Bhaskara: x = [-b ± sqrt(b² - 4ac)] / 2a. O valor dentro da raiz (b² - 4ac) é chamado de discriminante (Δ). Se Δ > 0, há duas raízes reais e distintas; se Δ = 0, há uma raiz real; se Δ < 0, não há raízes reais. Quer um exemplo de como aplicar a fórmula?";
            } else if (lowerCaseMessage.includes("leis de newton")) {
                response = "As Leis de Newton são três princípios fundamentais da física clássica: 1. **Inércia:** Um corpo em repouso tende a permanecer em repouso, e um corpo em movimento tende a permanecer em movimento com velocidade constante, a menos que uma força externa atue sobre ele. 2. **Dinâmica:** A força resultante que atua sobre um corpo é igual ao produto de sua massa pela sua aceleração (F = ma). 3. **Ação e Reação:** Para toda ação, há uma reação de igual intensidade e em sentido contrário. Qual lei você gostaria de entender melhor?";
            } else if (lowerCaseMessage.includes("tabela periódica")) {
                response = "A Tabela Periódica organiza os elementos químicos com base em seus números atômicos, configurações eletrônicas e propriedades químicas. Ela é dividida em períodos (linhas horizontais) e grupos (colunas verticais). Os elementos de um mesmo grupo geralmente possuem propriedades químicas semelhantes. É uma ferramenta essencial para entender a química. Quer saber sobre um elemento ou grupo específico?";
            } else if (lowerCaseMessage.includes("fotossíntese")) {
                response = "A fotossíntese é o processo pelo qual plantas, algas e algumas bactérias convertem energia luminosa em energia química. Elas utilizam dióxido de carbono (CO2) e água (H2O) para produzir glicose (açúcar) e oxigênio (O2), com a ajuda da clorofila. É fundamental para a vida na Terra, pois produz o oxigênio que respiramos e a base da cadeia alimentar. Quer detalhes sobre as fases clara e escura?";
            } else if (lowerCaseMessage.includes("revolução industrial")) {
                response = "A Revolução Industrial foi um período de grandes transformações econômicas e sociais, iniciado na Inglaterra no século XVIII, caracterizado pela substituição da produção artesanal pela maquinofatura, o uso do vapor como fonte de energia e o surgimento de novas tecnologias. Trouxe o crescimento das cidades, o surgimento de novas classes sociais (burguesia industrial e proletariado) e mudanças profundas nas relações de trabalho. Quer saber sobre suas fases ou impactos?";
            } else if (lowerCaseMessage.includes("globalização")) {
                response = "A globalização é um processo de aprofundamento da integração econômica, social, cultural e política entre os países. É impulsionada pelo avanço dos transportes e das comunicações, pela liberalização econômica e pela expansão das empresas transnacionais. Tem aspectos positivos, como o intercâmbio cultural e o acesso a novos mercados, e negativos, como o aumento das desigualdades e a padronização cultural. O que mais te interessa sobre a globalização?";
            } else {
